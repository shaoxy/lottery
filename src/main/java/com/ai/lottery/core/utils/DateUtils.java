...
public abstract class DateUtils {
    ...
    public static Date string2Date(String str, String format) {
        ...
        requireNonNull(format, "format is null");
        return DateTimeFormat.forPattern(format).parseDateTime(str).toDate();
    }
     ...
     *
     * @param date  传入日期
     * @param field 修改的字段，例如DateUtils.YEAR、DateUtils.MONTH等
     * @param i     移动的数量
     * @return 修改后的日期
     */
    ...
    public static boolean isAfter(Date date1, Date date2) {
        if (CheckNull.isNull(date1) || CheckNull.isNull(date2)) return false;
    ...
    public static int compareTo(Date date1, Date date2) {
        if (CheckNull.isNull(date1) && CheckNull.isNull(date2))
            return 0;
    ...
    public static boolean isSameDay(Date date1, Date date2) {
        ...
        int day1 = getDayOfYear(date1);

        int year2 = getYear(date2);
        int day2 = getDayOfYear(date2);
